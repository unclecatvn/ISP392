...
public class InsertCourse extends HttpServlet {
    ...
    throws ServletException, IOException {
        String courseName = request.getParameter("courseName").toUpperCase();
        String courseDescription = request.getParameter("courseDescription");
        char lastChar = courseName.charAt(courseName.length()-1);
        String result = courseName.substring(0, courseName.length()-1) + Character.toLowerCase(lastChar);
        CourseDAO dao = new CourseDAO();
        dao.insertCourse(result, courseDescription);
